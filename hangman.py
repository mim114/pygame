...
ворон выдра голубь гусь жаба зебра змея индюк кит кобра коза козел койот 
...
def displayBoard(missedLetters, correctLetters, secretWord):
    ...
    for letter in blanks: # показывает сикретное слово с пробелами между букв
        print(letter, end = ' ')
    print()
...
def playAgain():
    # эта функция возвращает значение True, если игрок хочет сыграть заново; в противном
    # случае возвращает False.
    print('Хочеш сыграть ещё? (да или нет?)')
    return input().lower().startswith('д')

print('В И С Е Л И Ц А(животные)')
missedLetters = ''
correctLetters = ''
...
while True:
    ...
    if guess in secretWord:
        ...
        # проверяет, превысил ли игрок лемит попыток и проиграл
        if len(missedLetters) == len(HANGMAN_PICS) - 1:
            displayBoard(missedLetters, correctLetters, secretWord)
            print('Вы исчерпали все поытки!\nнеугадано букв: ' + 
                  str(len(missedLetters)) + ' и угаданно букв:', 
